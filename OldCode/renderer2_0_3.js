...
async function generateAnkiCards() {
    ...
    media.forEach(slide => {
        ...
        // --- Original logic for generating standard cloze cards ---
        if (slide.questions) {
        ...
        // --- New logic for generating topic-based summary cards ---
        if (slide.topic && slide.topic.trim() !== '') {
            let questionList = '';
            if (slide.questions) {
                // Split questions, filter empty lines, then map over them to process
                questionList = slide.questions
                    .split('\n')
                    .filter(q => q.trim() !== '')
                    .map(q => {
                        // Use regex to find all cloze deletions and replace them with their content
                        // const strippedQuestion = q.replace(/\{\{c\d+::(.*?)\}\}/g, '$1');
                        const strippedQuestion = q.replace(/\{\{c\d+::(.*?)\}\}/g, '<b>$1</b>');
                        // Return the question with a bullet point
                        return `<br>• ${strippedQuestion.trim()}`;
                    })
                    .join(''); // Join the array into a single string
            }

            txtContent2 += `${slide.topic.trim()} ; <img src="${imageName}">${notesList} ; ${questionList} \n`;
        }
    });
