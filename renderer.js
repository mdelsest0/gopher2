...
async function generateQuestions(apiKey) {
    ...
    // Store the button's original text to restore it later
    const originalButtonText = generateQuestionsButton.textContent;
    ...
    try {
        ...
        generateQuestionsButton.textContent = originalButtonText;
        return;
    }

    const genAI = new GoogleGenerativeAI(apiKey);
    // Using a valid, available model. Replace with a different name when new models are released.
    const model = genAI.getGenerativeModel({ model: "gemini-2.5-flash-lite" });

    const totalSlides = media.length;
    let workingTopic = ''; // Variable to track the current topic across slides

    // Use a standard 'for' loop to get the index for the counter
    for (let i = 0; i < totalSlides; i++) {
        const slide = media[i];
        const currentSlideNumber = i + 1;

        // Update the button text to show progress
        generateQuestionsButton.textContent = `${currentSlideNumber}/${totalSlides}`;
        ...
        try {
            ...
            // Create the prompt dynamically, inserting the last known topic
            const prompt = `You are an expert academic assistant creating study materials for an MD/PhD student at the University of Florida. Your task is to analyze medical school lecture slides and generate high-yield content for Anki flashcards to help the student prepare for exams.

Crucial Rules:
1. The generated questions and content must be standalone and make sense without seeing the original slide image. Do not use phrases like "this slide," "the image shows," etc.
2. Focus exclusively on specific, testable information. If a slide contains no examinable content (e.g., it's a title or transition slide), return only the word "FILLER".

If this is a content slide (not a title, transition, or blank slide), generate the following sections. If it is a filler slide, return only the word "FILLER".

The previous topic was "${workingTopic}".
Topic: Analyze the slide's content. If it continues the previous topic, repeat it exactly. If it introduces a new concept, provide a new, general topic (3-5 words) that can cover this and subsequent slides.

Summary: A single, concise sentence that captures the main point of the slide. Do not use any bullets or special formatting.

Vocabulary: (Optional) If the slide contains complex medical or scientific terms, list and define them. List each term on a new line without any bullets or formatting.

Questions: Generate 0-6 high-yield, exam-style questions.
- Frame them as open-ended, conceptual questions that require recalling a specific term, pathway, or concept.
- IMPORTANT: Each question must be on a new line. Do not use bullet points, asterisks, numbers, or any leading characters or formatting.`;

            const result = await model.generateContent([prompt, imagePart]);
            const response = await result.response;
            const text = response.text();

            if (text.trim().toUpperCase() === 'FILLER') {
                console.log(`Slide marked as FILLER, skipping.`);
                // We still need to delay even if it's a filler slide
                await delay(2100*2);
                continue;
            }

            const topicMatch = text.match(/Topic:\s*([\s\S]*?)(?=\n\nSummary:|\n\nVocabulary:|\n\nQuestions:|$)/);
            const summaryBlockMatch = text.match(/Summary:\s*([\s\S]*?)(?=\n\nQuestions:|$)/);
            const questionsMatch = text.match(/Questions:\s*([\s\S]*)/);

            slide.topic = topicMatch ? topicMatch[1].trim() : (slide.topic || '');
            slide.notes = summaryBlockMatch ? summaryBlockMatch[1].trim() : (slide.notes || '');
            slide.questions = questionsMatch ? questionsMatch[1].trim() : (slide.questions || '');

            // Update workingTopic for the next iteration of the loop
            workingTopic = slide.topic;
            
        } catch (error) {
            console.error(`Error processing slide ${path.basename(filePath)}:`, error);
        }

        // Wait for 2.1 seconds before the next API call to stay within a 30 RPM limit
        await delay(2100);
    }

    try {
        await fs.writeFile(mediaFilePath, JSON.stringify(media, null, 2));
        console.log('media.json updated successfully with generated content.');
    } catch (error) {
        console.error('Error writing updated media.json:', error);
    }

    // Restore the button's original text when the process is complete
    generateQuestionsButton.textContent = originalButtonText;
}
...
async function generateAnkiCards() {
    ...
    let txtContent = tags;
    let txtContent2 = ""; // Initialize txtContent2 for topic-based cards
    const ankiMediaFolder = path.join(process.env.APPDATA, 'Anki2', 'User 1', 'collection.media');

    let workingTopic = ''; // Variable to track the current topic

    for (let i = 0; i < media.length; i++) {
        const slide = media[i];

        if (slide.topic && slide.topic.trim() !== '') {
            workingTopic = slide.topic.trim();
        }
        ...
        // --- Generate cloze cards for the CURRENT slide in the main loop ---
        if (slide.questions) {
            const notesList = slide.notes
                ? slide.notes.split('\n').filter(note => note.trim() !== '').map(note => `<br>• ${note}`).join('')
                : '';
            const topicHTML = workingTopic 
                ? `<br><b>${workingTopic}</b>` 
                : '';
            const questionsList = slide.questions.split('\n').filter(q => q.trim() !== '');
            questionsList.forEach(question => {
                txtContent += `${question.trim()} ; <img src="${imageName}"> ${topicHTML} ${notesList} \n`;
            });
        }

        // --- Logic for generating topic-based summary cards (txtContent2) ---
        if (slide.topic && slide.topic.trim() !== '') {
            const topic = slide.topic.trim();
            const topicImageName = imageName;
            const topicNotesList = slide.notes
                ? slide.notes.split('\n').filter(note => note.trim() !== '').map(note => `<br>• ${note}`).join('')
                : '';
            let allStrippedQuestions = [];

            const processQuestions = (currentSlide) => {
                if (!currentSlide.questions) return [];
                return currentSlide.questions
                    .split('\n')
                    .filter(q => q.trim() !== '')
                    .map(q => `<br>• ${q.replace(/\{\{c\d+::(.*?)\}\}/g, '<b>$1</b>').trim()}`);
            };

            // Add questions from the topic-starting slide
            allStrippedQuestions.push(...processQuestions(slide));

            // --- Look ahead to subsequent slides ---
            let nextIndex = i + 1;
            while (nextIndex < media.length && (!media[nextIndex].topic || media[nextIndex].topic.trim() === '')) {
                const nextSlide = media[nextIndex];
                
                // --- FIX: Generate cloze cards for this 'look-ahead' slide so it's not skipped ---
                if (nextSlide.questions) {
                    const nextImageName = path.basename(nextSlide.filePath);
                    const nextNotesList = nextSlide.notes
                        ? nextSlide.notes.split('\n').filter(note => note.trim() !== '').map(note => `<br>• ${note}`).join('')
                        : '';
                    const topicHTML = workingTopic ? `<br><b>${workingTopic}</b>` : '';

                    const questionsList = nextSlide.questions.split('\n').filter(q => q.trim() !== '');
                    //questionsList.forEach(question => {
                    //    txtContent += `${question.trim()} ; <img src="${nextImageName}"> ${topicHTML} ${nextNotesList} \n`;
                    //});
                }
                // --- End of Fix ---

                // Add its questions to the topic summary card
                allStrippedQuestions.push(...processQuestions(nextSlide));
                nextIndex++;
            }

            const questionList = allStrippedQuestions.join('');
            txtContent2 += `${topic} ; <img src="${topicImageName}">${topicNotesList} ; ${questionList} \n`;
    ...
    // --- Save the original questions file (txtContent) ---
    ...
    try {
        ...
        console.log(`Questions TXT file generated and saved successfully at ${outputFilePath}.`);
    } catch (error) {
        console.error('Error writing questions TXT file:', error);
    }

    // --- Save the new topics file (txtContent2) if it has content ---
    if (txtContent2.trim() !== '') {
        const outputFilePath2 = path.join(downloadsFolder, `${contentDetails.name}_topics.txt`);
        try {
            await fs.writeFile(outputFilePath2, txtContent2.trim(), 'utf8');
            console.log(`Topics TXT file generated and saved successfully at ${outputFilePath2}.`);
        } catch (error) {
            console.error('Error writing topics TXT file:', error);
        }
    }
...
async function openSlideViewer() {
    ...
    // --- FIX: Create topic input as a single-line <input> ---
    const topicInput = document.createElement('input');
    topicInput.type = 'text'; // Explicitly set the type
    topicInput.id = 'topic-input';
    topicInput.placeholder = 'Enter topic here...';
    topicInput.spellcheck = true;
    rightSideContainer.appendChild(topicInput);
    ...
    function updateSlideView() {
        ...
        topicInput.value = currentSlide.topic || '';
    }
    ...
    async function saveCurrentSlideData() {
        ...
        currentSlide.topic = topicInput.value;
    ...
    topicInput.addEventListener('input', saveCurrentSlideData);
}
